...
public class ImagePanelWithBuffering extends JPanel {
	...
	public void setImage(Image img, Image imageToPrepare) {
		
		calledImage=null; calledImage = img;
		directionHasChanged = true;
		
		if (! isImagingEngingBusy()) {
			setImageToShow(calledImage);
			
			// vergleiche neues calledImage mit ALTEM calledBufferImage
			if (calledImage.equals(calledBufferImage)) directionHasChanged = false;
			// nur wenn engineNotBusy wird Buffer-Bild gesetzt
			calledBufferImage = imageToPrepare;
	...
	public void paintComponent(Graphics g) {
		...
		if (imageToShow != null) {
			...
			calledBufferImage.flush(); calledImage.flush(); imageToShow.flush();
			setImage(calledImage); // BUG er kommt nach wie vor mit großen Bildern nicht zurecht!!!11
		}
	}
	...
	private void huntToOverrun() {
		weWereOverrun = false;
		setImage(calledImage, null); // Buffer = null, zum schnelleren Laden des callImage
		repaint();
	}
